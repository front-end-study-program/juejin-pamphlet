...
HTTP 协议规定，客户端支持的编码格式由Accept-Encoding指定。Accept-Encoding字段，有三个值，表示支持三种格式，分别是 gzip、deflate 和 br。
...
设置一个Content-Encoding响应头，告诉浏览器这个数据内容是经过压缩的。Content-Encoding响应头的值是对应的压缩算法名，那么浏览器就会调用相应的解压算法来对资源进行解压了。
